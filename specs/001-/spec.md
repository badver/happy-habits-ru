...
5. **Given** посетитель читает отзывы, **When** просматривает 6-8 текстовых и 2-3 видеоотзыва, **Then** каждый отзыв содержит формат «До/Запрос → Процесс → Результат/Сроки» с конкретными данными
...
- Как отображаются видеоотзывы на мобильном при медленном интернете? → Видео загружается с плейсхолдером, показываются субтитры, приоритет на текстовые отзывы
...
- **FR-003**: Каждый клик на кнопку мессенджера MUST отправлять событие аналитики в Яндекс.Метрику и Google Analytics с указанием позиции кнопки на странице
...
- **FR-012**: Блок «Отзывы и кейсы» MUST содержать 6-8 текстовых отзывов (400-700 символов каждый), 2-3 видеоотзыва (60-90 секунд с субтитрами), 2-3 анонимных кейса с диаграммой «уровень тревоги 0-10» на старте/после сессий, плашку «Все истории с согласия клиентов»
- **FR-013**: Блок «Стоимость и форматы» MUST содержать: цена индивидуальной онлайн-сессии, пакет из 10 онлайн-сессий с акцией (-10%, 1 сессия бесплатно), условия отмены (не позднее 24 часов), способы оплаты
...
**Формы и связь:**
- **FR-017**: Все контакты с психологом происходят ТОЛЬКО через Telegram или WhatsApp (формы обратной связи не используются)
- **FR-018**: Кнопки мессенджеров MUST открывать deeplink с предзаполненным сообщением для психолога
...
- **FR-022**: Система MUST отслеживать конверсионные события: клики на TG/WA, просмотр видео, скролл до ключевых блоков
- **FR-023**: Система MUST отслеживать глубину скролла и время на странице
- **FR-024**: Система MUST поддерживать A/B тестирование заголовков, текста CTA, порядка блоков
...
- **Отзыв (Testimonial)**: Социальное доказательство от клиента; атрибуты: текст (400-700 символов), формат (текст/видео), структура («До → Процесс → Результат/Сроки»), согласие клиента на публикацию
...
- **Аналитическое событие (Analytics Event)**: Действие посетителя, отслеживаемое Яндекс.Метрикой и Google Analytics; типы: messenger_click (TG/WA), video_play, scroll_75, time_60s; атрибуты: тип события, мессенджер, позиция кнопки, UTM-метки, временная метка
